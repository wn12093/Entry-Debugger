...
2. `check-page-core-loader.js`가 제품 코드와 분리된 복제품을 검증하는 테스트가 아닌지
   확인한다. content.js 심볼을 실제로 추출하고 문자열·주석 안의 구분자를 건너뛰는
   자체 scanner를 사용하지만, 정규식 리터럴이나 템플릿 보간 등에서 의미 있는 변경을
   놓칠 가능성이 있는지 판단한다.
3. page-core 테스트의 표현 수위가 정확한지 확인한다.
   - 현재 검사는 12개 기능 주입 경로의 코어 선행 호출과, 성공 경로의 DOM append
     순서·로딩 중 중복 가드·onload 정리만 다룬다.
...
5. manifest와 README 버전이 2.6.1로 일치하고 권한·대상 URL·WAR 범위가 넓어지지
   않았는지 확인한다. `package.json`에는 version 필드가 없으므로 세 파일의 버전이
   일치한다고 잘못 보고하지 않는다.
...
진행 UI를 검증한다. `forceResync()`로 치환된 10개 메시지 경로 전부를 실행하는
테스트는 아니므로, 10개 경로는 정적 동등성 확인과 일부 런타임 회귀를 구분해 보고한다.
파일이 없으면 미실행으로 기록하고 대체 검증을 제안한다.
