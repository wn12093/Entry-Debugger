...
| 2 | page-core 로딩 특성 테스트 | 부분 완료 | 낮음 | DOM 추가 순서·로딩 중 중복 방지·성공 후 정리 고정, 실패 복구는 후속 |
...
- `tools/check-page-core-loader.js`: 현행 코어 DOM 추가 순서·로딩 중 중복 방지·성공 후 정리
...
A2. content.js(2,779줄) god file → panel-ui / snapshot-render / feature-registry /
    message-router 분리.
A3. inject.js(1,206줄) 단일 onMessage 스위치(L1056~) → serializers / mutators / router 분리.
