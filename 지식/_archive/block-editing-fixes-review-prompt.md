...
## 기능 동작 검증 프롬프트 (AI 또는 수동)

위 코드 리뷰와 별개로, **두 기능이 실제 편집기에서 의도대로 동작하는지** 확인하는
콜드 프롬프트. 사용자 직접 검토와 병행한다.

```text
엔트리 디버거 확장에서 방금 수정한 두 기능이 실제 편집기에서 의도대로 동작하는지
playentry.org/ws에서 검증해줘. 브라우저를 제어할 수 있으면 단계대로 직접 수행하고,
아니면 사람이 그대로 따라 할 수 있게 단계와 기대 결과를 제시해. 코드는 수정하지 마.

[준비]
- 대상: https://playentry.org/ws (작품 만들기 편집기).
- 확장 로드: entry-debugger-extension(또는 dist/entry-debugger-extension-dev).
  팝업에서 "디버거 활성화" + "블록 텍스트 복사" + "Alt 단일 블록 드래그"를 켜고
  페이지를 새로고침한다.
- 두 기능 모두 기본 OFF이므로, 켠 뒤 "기능을 켜기 전부터 편집기에 있던 블록"에서도
  동작하는지가 이번 수정의 핵심이다.

[검증 1 — 블록 텍스트 복사: 만약~라면~아니면]
1) 블록을 다음과 같이 조립한다.
     시작하기 버튼을 클릭했을 때
     만일 (10 = 10) (이)라면
       이동 방향으로 (10) 만큼 움직이기
     아니면
       화면 끝에 닿으면 튕기기
2) 묶음 최상단 블록에서 우클릭 → "블록 텍스트 복사"를 누르고 메모장 등에 붙여넣는다.
PASS 기준:
  - "아니면"이 조건 머리줄과 다른 줄에 있고, 라면 본문과 아니면 본문 "사이"에 위치한다.
  - 즉 머리줄은 "만일 (10 = 10) (이)라면"으로 끝나야 한다.
FAIL 기준:
  - 머리줄이 "만일 (10 = 10) (이)라면 아니면"처럼 "아니면"이 붙어 나온다.
회귀 확인:
  - "아니면"이 없는 일반 "만일 ~라면" 복사가 정상인지.
  - 중첩 if/else, 비어 있는 아니면 분기, 반복 등 다른 statement 블록 복사가 그대로인지.

[검증 2 — Alt 단일 블록 드래그]
1) 한 줄로 이어진 블록 3개 이상을 만든다(예: 이동하기 → 기다리기 → 말하기).
   ※ 기능을 켜기 전부터 있던 블록으로 테스트한다(핵심).
2) 가운데 블록을 Alt 키를 누른 채 드래그한다.
PASS 기준:
  - 클릭한 한 블록만 떨어져 마우스를 따라오고, 그 아래 블록은 위 블록에 다시 붙어
    원래 자리에 남는다.
  - (대조군) Alt 없이 가운데 블록을 드래그하면 그 블록부터 아래 전부가 함께 이동한다.
FAIL 기준:
  - Alt를 눌러도 클릭 블록부터 아래 전부가 함께 이동한다(= 이번에 고친 증상 재현).
추가 확인:
  - 스택 첫 블록(맨 위)도 Alt+드래그로 단일 분리되는지.
  - Alt 단일 드래그 직후 Ctrl+Z가 한 번에 원래대로 되돌리는지.
  - 설정에서 "Alt 단일 블록 드래그"를 끄면 Alt+드래그가 다시 전체 스택 이동으로 돌아오는지.

[보고]
1. 검증 1·2 각각 PASS/FAIL과, 실제 복사된 텍스트 / 드래그 결과(가능하면 스크린샷).
2. FAIL이면 정확한 증상과 콘솔 오류 메시지.
3. 확장 미로드·외부 자원 차단 같은 환경 문제와 기능 실패를 구분해서 표기한다.
```
